            ...
            .then (matchlist=> {
                ...
                Promise.all(matchPromises).then(games => {
                    games.forEach(game =>{
                        ...
                        //console.log(game)
                        let id = jsonQuery('participantIdentities.player[summonerName='+req.params.name+']', { data: game});
...
function gameThing(game, name) {

    var id = (game['participantIdentities'].filter(x => x['player']['summonerName'].toUpperCase()==name.toUpperCase()))[0]['participantId'];
    var player = game['participants'].filter(x => x['participantId'] == id)[0];
    var teamID = player['teamId'];
...
router.get('/:name', (req, res) => {
    return kayn.Summoner.by.name(req.params.name)
      .then(summoner => kayn.Matchlist.by.accountID(summoner['accountId']).region(REGIONS.NORTH_AMERICA))
      .then(matchlist=> Promise.all(matchlist.matches.slice(0,1).map(match => kayn.Match.get(match.gameId))))
      .then(games => games.map(game => gameThing(game, req.params.name)))
      .then((matchesReturns) => res.status(200).json(matchesReturns))
      .catch(error => {
